...
Pagination is available to retrieve some portion of the results from a GET query.
In this case the returned file contains a subset of elements of the complete results. The file is stand-alone, meaning that it has a root element and the same structure as a un-paginated file.
...
Notice the response headers. When the "X-Hits" header is 0, there are no elements left or the scrollId is not valid (anymore).
