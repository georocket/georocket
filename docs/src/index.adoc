...
In addition to tags, you may attach properties to imported files. Properties are key:value pairs including arbitrary information that can be queried in later searches. Multiple properties are separated by comma, each pair is separated by a colon. GeoRocket supports dates as values, which are encoded as follows: 

`yyyy-MM-ddTHH:mm:ss` (e.g. 2017-03-15[T13:40:25]). 

An example for properties attached to an import is:

  georocket import --properties creation-date:2017-03-15,author:mustermann myfile.xml
...
Append one or multiple tags (separated by comma) to existing chunks in the GeoRocket data store. Provide a <<query-language, query>> to specify the chunks, whose tags should be updated:
    
    georocket appendTag --tags tag1,tag2 myquery
    
This example appends the tags `tag1` and `tag2` to chunks matching the query `myquery`.

You can also restrict the command to a certain layer.

    georocket appendTag --layer CityModel/LOD1 --tags tag1,tag2 myquery

This example appends the tags `tag1` and `tag2` to chunks within the layer `CityModel/LOD1` matching the query `myquery`.
...
Remove one or multiple tags (separated by comma) from existing chunks in the GeoRocket data store. Provide a <<query-language, query>> to specify the chunks, whose tags should be updated:
    ...
    georocket removeTag --tags tag1,tag2 myquery
...
You can also restrict the command to a certain layer.
    ...
    georocket removeTag --layer CityModel/LOD1 --tags tag1,tag2 myquery
...
Examples:
  ...
  LT(year-of-construction 1990)
  GT(building-height 50)

Comparison operations cannot be nested. GeoRocket support the comparison of strings, numbers and dates. 

==== EQ
Use the equal comparison to search for chunks that includes a property equally to the search. Example:

    EQ(foo bar)

This example matches all chunks that have a property with key=foo and value=bar.

==== LT
Use the less than comparison to search for chunks that includes a property matching the key and a value less than the one given within the query. Example:
    
    LT(foo 12)

This example matches all chunks that have a property with key=foo and those values are less than 12. Besides numbers, dates can be compared, too. Example:

    LT(foo 2017-01-12)
    
This matches all chunks having a property with key=foo and those values are dates earlier than 2017-01-12. Additionally, a time can be specify such as `2017-01-12T09:15:59`.  

==== LTE
Use the less than or equal comparison to search for chunks that includes a property matching the key and a value less than or equal to the one given within the query. Example:
    
    LTE(foo 12)

This example matches all chunks that have a property with key=foo and those values are less than or equal to 12. Besides numbers, dates can be compared, too.

==== GT
Use the greater than comparison to search for chunks that includes a property matching the key and a value greater than the one given within the query. Example:
    
    GT(foo 12)

This example matches all chunks that have a property with key=foo and those values are greater than 12. Besides numbers, dates can be compared, too.

==== GTE
Use the greater than or equal than comparison to search for chunks that includes a property matching the key and a value greater than or equal to the one given within the query. Example:
    
    GTE(foo 12)

This example matches all chunks that have a property with key=foo and those values are greater than or equal to 12. Besides numbers, dates can be compared, too.
