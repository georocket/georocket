...
With scrolling the returned file is also stand-alone, meaning that it has a root element and the same structure as a un-paginated file.
...
The default page size is 100. It can be changed by providing a size query
argument with the initial scrolling erquest. After the scroll has started, the
size will have no effect on further requests.

===== Example request

  GET /store/CityModel?scroll=true&size=75&search=LOD1+textured+13.378,52.515,13.380,52.517 HTTP/1.1
