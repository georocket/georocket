...
public class StoreEndpointTest {
  ...
  public void testPaginationWithGivenScrollId(TestContext context) {
    ...
    doPaginatedStorepointRequest(context, "/?search=DUMMY_QUERY&paginated=true&scrollId=" + MockIndexer.FIRST_RETURNED_SCROLL_ID, true, true, response -> {
      context.assertEquals(MockIndexer.LAST_RETURNED_SCROLL_ID, response.getHeader(HeaderConstants.SCROLL_ID), "The second scrollId should be invalid if there a no elements left.");
      checkGeoJsonResponse(response, context, returned -> {
  ...
  public void testPaginationWithInvalidScrollId(TestContext context) {
    ...
    doPaginatedStorepointRequest(context, "/?search=DUMMY_QUERY&paginated=true&scrollId=" + MockIndexer.INVALID_SCROLLID, false, false, response -> {
  ...
  /**
   * Checks for pagination-specific headers that are returned from the server are present or not.
   * @param response
   * @param context
   */
  private void checkPaginatedResponsePresent(HttpClientResponse response, TestContext context, Boolean checkScrollIdHeaderPresent) {
  ...
  private void doPaginatedStorepointRequest(TestContext context, String url, Boolean checkHeaders, Boolean checkScrollIdHeaderPresent, Handler<HttpClientResponse> handler) {
    ...
    HttpClientRequest request = client.get(url, response -> {
      if (checkHeaders) {
        checkPaginatedResponsePresent(response, context, checkScrollIdHeaderPresent);
      }
  ...
  private final class HeaderConstants {
    ...
    static final String PAGE_SIZE = "X-Page-Size";
  }
}
