...
where
    C: Stream<Item = crate::post_gis::Chunk> + Unpin,
    W: AsyncWrite + Unpin,
    M: MergeMeta,
{
    let mut count = 0;
...
mod tests {
    ...
    fn chunk_from_str(s: &str) -> crate::post_gis::Chunk {
        Ok(s.to_string())
    }
    ...
    async fn test_merge_empty() {
        let chunks: Vec<crate::post_gis::Chunk> = vec![];
    ...
    async fn test_merge_one() {
        ...
        let chunks: Vec<crate::post_gis::Chunk> = once(chunk).map(chunk_from_str).collect();
        let chunks = futures_util::stream::iter(chunks);
        let mut dest = vec![];
    ...
    async fn test_merge_multiple() {
        ...
        let chunks: Vec<crate::post_gis::Chunk> =
            repeat(chunk).take(3).map(chunk_from_str).collect();
        let chunks = futures_util::stream::iter(chunks);
        let mut dest = vec![];
        let count = merge_into(chunks, &mut dest, GeoJsonFeatureCollection)
            .await
