...
impl GeoDataImporter {
    ...
    pub async fn run(mut self) -> anyhow::Result<()> {
        let splitter_handle = spawn(async move { self.splitter.run().await });
        let store_handle = spawn(async move { self.store.run().await });
        let index_handle = spawn(self.indexer.run());
