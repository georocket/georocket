...
mod tests {
    ...
    async fn simple_features_select_all_bbox() {
        let (_container, client) = setup("test_files/simple_features_postgis.sql").await;
        let bounding_box = BoundingBox::from_tuple((0.0, 0.0, 10.0, 10.0, Some(4326)));
    ...
    async fn simple_feature_select_some_bbox() {
        let (_container, client) = setup("test_files/simple_features_postgis.sql").await;
        let bounding_box = BoundingBox::from_tuple((0.0, 0.0, 5.0, 8.0, Some(4326)));
        let query = query![bounding_box];
        let chunks: Vec<String> = query_client(&client, query)
            .await
            .unwrap()
    ...
    async fn simple_feature_select_and_bbox() {
        let (_container, client) = setup("test_files/simple_features_postgis.sql").await;
        let bounding_box_a = BoundingBox::from_tuple((0.0, 0.0, 9.0, 5.0, Some(4326)));
        let bounding_box_b = BoundingBox::from_tuple((4.0, 3.0, 9.9, 9.0, Some(4326)));
